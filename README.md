...
- `apps/market/src/catalogue.js` — **111** professional expert prompts and **14** expert teams with source-audit records (including 12 Unity experts, 2 Unity teams, the general expert team, and 9 domain collaboration teams).
...
The local catalogue now ships **111 experts**: the original 24 general experts (evidence research, architecture, testing, security, delivery, planning, docs, frontend, Git, performance, and more), **12 Unity experts**, and a **75-expert expansion batch** adapted from six additional MIT skill collections — backend/API/database/data pipeline, cloud/Kubernetes/IaC/networking, incident response and log forensics, mobile/design-system/accessibility, LLM application/prompt/vector search/MLOps, business analysis/growth content/SEO, technical translation/diagrams/slides, scientific methodology/statistics/peer review, plus **PC troubleshooting with disk-space optimization**. The catalogue also includes a general expert team that read-only scans globally and project-installed DSH Skills at the start of each task, identifies every single-expert Skill, and routes the request to the smallest sufficient set based on relevance and risk before a final integration pass. Example teams cover full-stack delivery, frontend delivery (UI designer + design system), data platform, incident response, AI product lab, growth content, science review, platform migration, and PC rescue. They reference publicly licensed GitHub Skills/Agents, but the prompts are rewritten by this project and are not direct installs of the source repos. Every adapted entry shows its source file, pinned commit, and license in the detail panel.
...
# Install the general on-demand expert team into DSH global Skills
...
`team.general-expert-team` is the default cross-domain entry point. At the start of each task it automatically discovers all single-expert Skills installed in DSH's global and current-project directories; a project-local Skill overrides a global Skill with the same name. The team declares no member overrides: the Coordinator Agent builds the member graph at runtime from the experts actually installed, evaluates the original request, candidate capabilities, relevance, and risk, then invokes the smallest sufficient set. If discovery fails, it reports the gap explicitly before the final integration pass. Discovery reads metadata only, does not execute unfamiliar content, and cannot expand DSH permissions. `team.unity-development` targets concrete development tasks with candidate roles including architecture, Gameplay, Editor, data, assets, physics/navigation, performance, and QA; each run has the Coordinator Agent decide which roles are actually needed, and an integration role closes the loop. `team.unity-game-production` targets end-to-end production from concept to release with candidate roles including production coordination, game design, technical architecture, content/assets, performance, QA, and build & release. A team Skill embeds each member's actual prompt payload and the coordination protocol into a single `SKILL.md`; the host must inject that payload together with the original task, assignment, upstream results, and locale on every call. When the host provides `TeamCoordinator` and `ExpertExecutor`, `TeamRunner` from `@expert/core` validates dispatch decisions, runs per-round calls in parallel, and records both real invocations and skips.
